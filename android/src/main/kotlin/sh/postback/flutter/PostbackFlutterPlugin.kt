...
class PostbackFlutterPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    private fun newBridgeExecutor(): ExecutorService = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "PostbackFlutterBridge").apply { isDaemon = true }
    }

    private fun sdk(): Postback = Postback.shared(context)
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        channel = MethodChannel(binding.binaryMessenger, "postback_flutter/native")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "configure" -> {
                ...
                if (apiKey.isEmpty()) {
                    result.error("CONFIGURE_ERROR", "Postback.configure requires a non-empty apiKey.", null)
                    return
                }
            ...
            "enableAppleAdsAttribution" -> {
                // Apple Ads is only available through the iOS bridge.
                result.success(false)
            }

            "getPostbackId" -> result.success(sdk().getPostbackId())
